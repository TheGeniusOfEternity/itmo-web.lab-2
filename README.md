...
### Диапазоны значений
- Изменение х (Checkbox): -2, -1.5, -1, -0.5, 0, 0.5, 1, 1.5, 2
- Изменение y (Text): (-3, 3)
- Изменение R (Select): 1, 2, 3, 4, 5

### Конфигурация сервера WildFly:
1) Скачать WildFly: ```wget <link for wildfly>```
2) Распаковать с помощью команд _unzip / tar_: ```unzip wildfly```
3) Загрузить WAR-архив с приложением в ```wildfly/deployments/ROOT.war```
4) Запустить: ```~/wildfly/bin/standalone.sh -c standalone.xml -b 0.0.0.0 -Djboss.socket.binding.port-offset=<offset_value>```
